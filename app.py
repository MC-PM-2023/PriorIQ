...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, pwd):
            ...
            return redirect('/file')             # Normal users -> File Page
...
# --- patentryx (uniform status + validations; TF-IDF logic unchanged) ---
...
# --- bothranking (uniform status + validations; core logic unchanged) ---
...
# (We *use* load_df_stash / cleanup_stash below; if they’re not present in your app, the no-op fallbacks defined here will keep things running.)
...
def overall():
    ...
    sites = [
        ...
        {"label": "Rating",    "url": url_for('rating')},
        {"label": "Result",    "url": url_for('result')},
    ]
